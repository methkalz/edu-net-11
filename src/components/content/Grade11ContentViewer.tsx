  ...
  return <div className="space-y-8">
      ...
      <div className="space-y-6">
        {sections.length === 0 ? <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">لم يتم إضافة أي محتوى للصف الحادي عشر حتى الآن</p>
          </div> : sections.map(section => <Card key={section.id} className="border-l-4 border-l-blue-500">
              <Collapsible defaultOpen>
                ...
                <CollapsibleContent>
                  <CardContent>
                    <div className="space-y-4">
                      {section.topics.length === 0 ? <div className="text-center py-6 text-muted-foreground">
                          <FileText className="h-8 w-8 mx-auto mb-2 opacity-50" />
                          <p>لا توجد مواضيع في هذا القسم</p>
                        </div> : section.topics.map(topic => <Card key={topic.id} className="bg-gradient-to-r from-blue-50 to-green-50 border-blue-200">
                              ...
                              <div className="space-y-3">
                                <div className="flex items-start justify-between">
                                  <div className="flex-1">
                                    ...
                                    {topic.content && <div className="text-muted-foreground mb-3 leading-relaxed">
                                        <p>{topic.content}</p>
                                      </div>}
                                  </div>
                                  <Button variant="outline" size="sm" onClick={() => logger.debug('Topic view requested', {
                          topicId: topic.id
                                 ...
                                 {/* Lessons */}
                                {topic.lessons && topic.lessons.length > 0 && <div className="space-y-3">
                                    <h5 className="text-sm font-medium text-blue-800">الدروس:</h5>
                                    <div className="space-y-4">
                                      {topic.lessons.map(lesson => <div key={lesson.id} className="border-l-4 border-l-blue-300 pl-4 py-3 bg-white rounded-lg border border-blue-200 shadow-sm">
