...
import { Grade10LessonMedia } from '@/hooks/useGrade10AdminContent';
...
interface Grade10LessonMediaManagerProps {
  lessonId?: string;
...
const Grade10LessonMediaManager: React.FC<Grade10LessonMediaManagerProps> = ({
  lessonId,
  media,
  onAddMedia,
  onDeleteMedia,
  ...
  // Create callback to refresh data after Lottie/Code update
  const handleDataRefresh = () => {
    console.log('Triggering data refresh after media update');
    if (onUpdateMedia) {
      // This will trigger fetchAllContent in the parent component
      onUpdateMedia('', {}).catch(err => console.log('Refresh triggered'));
    }
  };
  
  const { updateLottieMedia } = useEditLottieMedia(handleDataRefresh, 'grade10_lesson_media');
  const { updateCodeMedia } = useEditCodeMedia(handleDataRefresh, 'grade10_lesson_media');
  ...
  const [editingLottie, setEditingLottie] = useState<Grade10LessonMedia | null>(null);
  const [editingCode, setEditingCode] = useState<Grade10LessonMedia | null>(null);

  const canEditMedia = userProfile?.role === 'superadmin';
  ...
  useEffect(() => {
    if (previewMedia && media) {
      ...
      if (updatedMedia && JSON.stringify(updatedMedia.metadata) !== JSON.stringify(previewMedia.metadata)) {
        console.log('Updating preview media in Grade10LessonMediaManager:', updatedMedia);
  ...
  useEffect(() => {
    if (editingCode && media) {
      ...
      if (updatedMedia && JSON.stringify(updatedMedia.metadata) !== JSON.stringify(editingCode.metadata)) {
        console.log('Updating editing code in Grade10LessonMediaManager:', updatedMedia);
  ...
  useEffect(() => {
    if (editingLottie && media) {
      ...
      if (updatedMedia && JSON.stringify(updatedMedia.metadata) !== JSON.stringify(editingLottie.metadata)) {
        console.log('Updating editing lottie in Grade10LessonMediaManager:', updatedMedia);
  ...
  const handleUpdateLottieMedia = async (updates: Partial<Grade10LessonMedia>) => {
    if (!editingLottie) return;
    
    try {
      console.log('Starting Lottie media update process...');
      
      // تحديث قاعدة البيانات (يستخدم grade10_lesson_media)
  ...
  const handleUpdateCodeMedia = async (updates: Partial<Grade10LessonMedia>) => {
  ...
  const handleAddMedia = async (mediaData: any, type: 'video' | 'image' | 'lottie' | 'code' | 'audio') => {
    ...
    try {
      ...
      const newMedia: Omit<Grade10LessonMedia, 'id' | 'created_at'> = {
        lesson_id: lessonId,
        media_type: type,
        file_path: filePath,
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="flex-1 min-w-0">
                   ...
                   <div className="flex gap-2">
                     ...
                     >
                      ...
                      {item.media_type === 'lottie' && canEditMedia && (
                        <Button
                          variant="ghost"
                          size="sm"
                      ...
                      {item.media_type === 'code' && canEditMedia && (
                        <Button
                          variant="ghost"
                          size="sm"
      ...
      {editingLottie && (
        <LottieEditForm
          media={editingLottie as any}
          isOpen={true}
          onClose={() => setEditingLottie(null)}
          onUpdate={handleUpdateLottieMedia as any}
        />
      )}
      ...
      {editingCode && (
        <CodeEditForm
          media={editingCode as any}
          isOpen={true}
          onClose={() => setEditingCode(null)}
          onUpdate={handleUpdateCodeMedia as any}
        />
      )}
    </div>
  );
...
export default Grade10LessonMediaManager;
