...
const Grade10MediaPreview: React.FC<Grade10MediaPreviewProps> = ({
  media,
  onClose
}) => {
  ...
  const getMediaTypeBadge = (type: string) => {
    ...
    return <Badge className={`${config.color} text-xs`}>
        {config.label}
      </Badge>;
  };
    ...
    switch (media.media_type) {
      case 'video':
        if (media.file_path.includes('youtube.com') || media.file_path.includes('youtu.be')) {
          const videoId = media.file_path.includes('youtube.com') ? media.file_path.split('v=')[1]?.split('&')[0] : media.file_path.split('youtu.be/')[1]?.split('?')[0];
          return <div className="aspect-video rounded-lg overflow-hidden">
              <iframe src={`https://www.youtube.com/embed/${videoId}`} title={media.file_name} className="w-full h-full" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
            </div>;
        } else if (media.file_path.includes('drive.google.com')) {
          const fileId = media.file_path.match(/[-\w]{25,}/)?.[0];
          return <div className="aspect-video rounded-lg overflow-hidden">
              <iframe src={`https://drive.google.com/file/d/${fileId}/preview`} title={media.file_name} className="w-full h-full" frameBorder="0" allow="autoplay" />
            </div>;
        } else {
          return <div className="aspect-video rounded-lg overflow-hidden bg-black">
              <video controls className="w-full h-full" src={media.file_path}>
      ...
      case 'image':
        return <div className="flex justify-center">
            <img src={media.file_path} alt={media.file_name} className="max-w-full max-h-[70vh] object-contain rounded-lg" />
          </div>;
      case 'lottie':
        ...
        try {
          ...
          if (media.metadata?.animation_data) {
            ...
            animationData = typeof media.metadata.animation_data === 'string' ? JSON.parse(media.metadata.animation_data) : media.metadata.animation_data;
          } else if (media.metadata?.lottie_data) {
            console.log('Using metadata.lottie_data');
            animationData = typeof media.metadata.lottie_data === 'string' ? JSON.parse(media.metadata.lottie_data) : media.metadata.lottie_data;
            ...
            return <div className="relative rounded-lg bg-amber-50 p-4 text-center">
          ...
          return <div className="relative rounded-lg overflow-hidden bg-gray-50 flex items-center justify-center p-4">
              <div className="w-full max-w-2xl min-h-[300px] max-h-[600px] flex items-center justify-center">
                <Lottie lottieRef={lottieRef} animationData={animationData} loop={loopSetting} autoplay={true} style={{
                width: '100%',
                height: '100%',
                maxWidth: '100%',
                maxHeight: '100%'
          ...
          return <div className="relative rounded-lg bg-red-50 border border-red-200 p-4 text-center">
      ...
      case '3d_model':
        ...
        return <ThreeDModelViewer modelUrl={media.file_path} modelType={modelType} title={media.file_name} showControls={true} autoRotate={media.metadata?.autoRotate !== false} className="w-full" />;
      case 'code':
        const codeContent = media.metadata?.code_content || '';
        const language = media.metadata?.language || 'text';
        return <div className="bg-gray-900 rounded-lg p-4 overflow-auto max-h-[60vh]">
