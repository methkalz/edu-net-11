...
const Grade11Content = () => {
  ...
  const filteredSections = useMemo(() => {
    ...
    // Apply search filter
    if (searchTerm) {
      filtered = filtered.filter(section => section.title.toLowerCase().includes(searchTerm.toLowerCase()) || section.description?.toLowerCase().includes(searchTerm.toLowerCase()) || section.topics.some(topic => topic.title.toLowerCase().includes(searchTerm.toLowerCase()) || topic.content?.toLowerCase().includes(searchTerm.toLowerCase()) || topic.lessons?.some(lesson => lesson.title.toLowerCase().includes(searchTerm.toLowerCase()) || lesson.content?.toLowerCase().includes(searchTerm.toLowerCase()))));
    }
  ...
  const statistics = useMemo(() => {
    ...
    const lessonsCount = filteredSections.reduce((acc, section) => acc + section.topics.reduce((topicAcc, topic) => topicAcc + (topic.lessons?.length || 0), 0), 0);
    return {
      sectionsCount,
      topicsCount,
      lessonsCount
  ...
  return <div className="space-y-8">
      ...
      <Tabs defaultValue="textual" className="w-full">
        ...
        <TabsContent value="files" className="space-y-6">
          <div className="space-y-4">
            ...
            <Grade11FileLibrary documents={documents} loading={filesLoading} onAddDocument={canManageContent ? handleAddDocument : () => {}} onEditDocument={canManageContent ? handleEditDocument : () => {}} onDeleteDocument={canManageContent ? deleteDocument : () => {}} uploadFile={uploadFile} addDocuments={addDocuments} getFileUrl={getFileUrl} />
          </div>
        </TabsContent>
      ...
      {showTopicForm && <Grade11TopicForm topic={editingTopic} sectionId={selectedSectionId} onSave={handleSaveTopic} onCancel={handleCloseForm} />}

      {showLessonForm && <Grade11LessonForm lesson={editingLesson} topicId={selectedTopicId} onSave={handleSaveLesson} onAddMedia={addLessonMedia} onDeleteMedia={deleteLessonMedia} onUpdateMedia={updateLessonMedia} onCancel={handleCloseForm} />}

      {showDocumentForm && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <Grade11DocumentForm onSave={handleSaveDocument} onCancel={handleCloseForm} initialData={editingDocument} />
        </div>}
