...
export function SmartQuestionGenerator({
  ...
  const {
    data: sections = []
  } = useGradeSections(gradeLevel);
  const {
    data: topics = []
  } = useSectionTopics(gradeLevel, sectionId);
  const {
    data: lessons = []
  } = useSmartGenTopicLessons(selectedTopicIds);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('question_bank').select('question_text').eq('grade_level', gradeLevel).eq('section_name', selectedSection.title).eq('is_active', true);
      return data || [];
    },
  ...
  const canGenerate = gradeLevel && sectionId && selectedTopicIds.length > 0 && selectedLessonIds.length > 0 && questionTypes.length > 0;
  ...
  const proceedWithGeneration = async (
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-smart-questions', {
        body: {
          gradeLevel: gradeLevel,
          sectionName: selectedSection.title,
          topicName: selectedTopicsNames,
          lessonId: actualLessonIds[0],
  ...
  const handleSaveApproved = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('question_bank').insert(questionsToSave.map(q => ({
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {step === 'setup' ? <div className="space-y-6 py-4">
            ...
            value: t.id,
            label: t.title
          }))]} value={selectedTopicIds} onChange={handleTopicChange} placeholder="اختر الموضوع أو المواضيع" searchPlaceholder="بحث في المواضيع..." />
                {selectedTopicIds.length > 0 && !selectedTopicIds.includes('all') && <p className="text-xs text-muted-foreground mt-1">
                    تم اختيار {selectedTopicIds.length} من {topics.length} مواضيع
                  </p>}
                {selectedTopicIds.includes('all') && <p className="text-xs text-muted-foreground mt-1">
                    تم اختيار كل المواضيع ({topics.length} مواضيع)
                  </p>}
              </div>}
            
            {/* اختيار الدروس */}
            {selectedTopicIds.length > 0 && <div>
                <Label>الدروس * (يمكن اختيار أكثر من درس - فقط الدروس المناسبة)</Label>
                <MultiSelect options={[{
            value: 'all',
            label: '📚 كل الدروس المناسبة'
          }, ...suitableLessons.map(l => ({
            value: l.id,
            label: l.title
          }))]} value={selectedLessonIds} onChange={handleLessonChange} placeholder="اختر الدرس أو الدروس" searchPlaceholder="بحث في الدروس..." />
                {selectedLessonIds.length > 0 && !selectedLessonIds.includes('all') && <p className="text-xs text-muted-foreground mt-1">
                    تم اختيار {selectedLessonIds.length} من {suitableLessons.length} دروس مناسبة
                  </p>}
                {selectedLessonIds.includes('all') && <p className="text-xs text-muted-foreground mt-1">
                    تم اختيار كل الدروس المناسبة ({suitableLessons.length} دروس)
                  </p>}
                {lessons.length > suitableLessons.length && <p className="text-xs text-orange-600 mt-1">
            ...
            <div>
              ...
              <Input id="question-count" type="number" min={1} max={50} value={questionCount} onChange={e => {
            const value = parseInt(e.target.value);
            if (!isNaN(value) && value >= 1 && value <= 50) {
              setQuestionCount(value);
            }
          }} placeholder="أدخل عدد الأسئلة (1-50)" className="text-center" />
            ...
            {/* توزيع الصعوبة */}
            <DifficultyDistributionSelector totalQuestions={questionCount} value={difficultyDistribution} onChange={setDifficultyDistribution} />
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Checkbox id="multiple_choice" checked={questionTypes.includes('multiple_choice')} onCheckedChange={checked => {
                if (checked) {
                ...
                <div className="flex items-center gap-2">
                  <Checkbox id="true_false" checked={questionTypes.includes('true_false')} onCheckedChange={checked => {
                if (checked) {
              ...
              {questionTypes.length === 0 && <p className="text-sm text-orange-600 mt-2">يرجى اختيار نوع واحد على الأقل</p>}
            </div>
            
            {/* تحذيرات */}
            {selectedLessonIds.length === 0 && selectedTopicIds.length > 0 && suitableLessons.length === 0 && <div className="flex items-start gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                {generatedQuestions.filter(q => q.similarityWarning?.found).length > 0 && <Badge variant="secondary">
                    {generatedQuestions.filter(q => q.similarityWarning?.found).length} سؤال مشابه
                  </Badge>}
              </div>
              
              <div className="flex gap-2">
                {generatedQuestions.some(q => q.similarityWarning?.found) && <Button type="button" size="sm" variant={showOnlyNew ? "default" : "outline"} onClick={() => setShowOnlyNew(!showOnlyNew)}>
                    <Filter className="w-4 h-4 mr-1" />
                    {showOnlyNew ? 'عرض الكل' : 'إخفاء المشابهة'}
                  </Button>}
                <Button type="button" size="sm" variant="outline" onClick={handleApproveAll} disabled={approvedQuestions.size === generatedQuestions.length}>
                  الموافقة على الكل
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {generatedQuestions.filter(q => !showOnlyNew || !q.similarityWarning?.found).map((question, idx) => <GeneratedQuestionCard key={idx} question={question} index={idx} onEdit={edited => handleEditQuestion(idx, edited)} onDelete={() => handleDeleteQuestion(idx)} onApprove={() => handleApproveQuestion(idx)} isApproved={approvedQuestions.has(idx)} />)}
            </div>
            ...
            <div className="flex justify-between gap-2 pt-4 border-t">
              ...
              <Button type="button" onClick={handleSaveApproved} disabled={approvedQuestions.size === 0 || isSaving}>
                {isSaving ? <>
                    <Loader2 className="w-4 h-4 ml-2 animate-spin" />
                    جاري الحفظ...
