...
const ModernHeader: React.FC<ModernHeaderProps> = ({ 
  ...
  const getLogoSize = () => {
    switch (headerSettings.logo_size) {
      case 'small': return 'h-16 w-auto';
      case 'medium': return 'h-20 w-auto';
      case 'large': return 'h-28 w-auto';
      default: return 'h-20 w-auto';
    }
  };
          ...
          <div className="flex items-center justify-center flex-1">
            {headerSettings.show_logo && (
              <img 
                ...
                alt="شعار النظام" 
                className={`${getLogoSize()} object-contain shadow-sm hover:scale-105 transition-transform duration-300`}
              />
            )}
          </div>
