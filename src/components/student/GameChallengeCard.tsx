...
const GameChallengeCard: React.FC<GameChallengeCardProps> = ({ topicId }) => {
  const { linkedGame, isLocked, isCompleted, bestScore, remainingPrereqs, loading } = useLessonGame(topicId);
      ...
      <Dialog open={showGame} onOpenChange={setShowGame}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto p-0 sm:p-6">
          <Suspense fallback={
            ...
            <PairMatchingGame gameId={linkedGame.id} />
          </Suspense>
        </DialogContent>
      </Dialog>
    </>
