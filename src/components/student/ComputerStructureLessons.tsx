    ...
    return (
      <div className="space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-bold text-foreground dark:text-foreground">مبنى الحاسوب</h2>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-blue-50 to-blue-100/50 dark:from-blue-500/10 dark:to-blue-500/5 border-blue-200/60 dark:border-blue-500/20">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
            ...
            <div className="text-xl font-bold text-blue-600 dark:text-blue-400 mb-0.5">{computerSections.length}</div>
        ...
        <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-purple-50 to-purple-100/50 dark:from-purple-500/10 dark:to-purple-500/5 border-purple-200/60 dark:border-purple-500/20">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
            ...
            <div className="text-xl font-bold text-purple-600 dark:text-purple-400 mb-0.5">{computerLessonsCount}</div>
        ...
        <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-orange-50 to-orange-100/50 dark:from-orange-500/10 dark:to-orange-500/5 border-orange-200/60 dark:border-orange-500/20">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
            ...
            <div className="text-xs text-orange-600/70 dark:text-orange-400/70 font-medium">نماذج ثلاثية الأبعاد</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-3">
        {computerSections.length === 0 ? (
          <Card className="text-center py-12 bg-muted/30 dark:bg-muted/10 border-0 transition-colors duration-300">
            <div className="space-y-3">
              <FolderOpen className="w-12 h-12 mx-auto text-muted-foreground/60 dark:text-foreground/40" />
              <h3 className="text-base font-medium text-foreground dark:text-foreground">لا توجد نتائج</h3>
              <p className="text-sm text-muted-foreground dark:text-foreground/70 max-w-md mx-auto">
          ...
          computerSections.map((section) => (
            <Card key={section.id} className="overflow-hidden border-0 shadow-sm hover:shadow-md dark:hover:shadow-primary/10 transition-all duration-300 bg-white/60 dark:bg-card/60 backdrop-blur-sm">
              <Collapsible 
                open={openSections.includes(section.id)}
                onOpenChange={() => toggleSection(section.id)}
              >
                <CollapsibleTrigger asChild>
                  <CardHeader className="cursor-pointer hover:bg-muted/20 dark:hover:bg-muted/10 transition-colors py-3 px-4">
                     <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <CardTitle className="text-xl font-semibold text-foreground dark:text-foreground">{formatSectionTitle(section.title)}</CardTitle>
                          {section.description && (
                            <p className="text-base text-muted-foreground dark:text-foreground/70 mt-0.5 line-clamp-1">
                              {section.description}
                            </p>
                          )}
                        </div>
                  ...
                  <CardContent className="pt-0 pb-3 px-4">
                    <div className="space-y-2">
                      {section.topics.map((topic) => (
                        <Card key={topic.id} className="ml-3 border-0 bg-muted/30 dark:bg-muted/20 transition-colors duration-300">
                          <Collapsible
                            open={openTopics.includes(topic.id)}
                            onOpenChange={() => toggleTopic(topic.id)}
                          >
                            <CollapsibleTrigger asChild>
                              <CardHeader className="cursor-pointer hover:bg-muted/40 dark:hover:bg-muted/30 transition-colors py-2.5 px-3">
                                 <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2.5">
                                    <div className="w-6 h-6 bg-gradient-to-r from-green-500 to-emerald-500 rounded-md flex items-center justify-center shadow-md">
                                      <Target className="w-3 h-3 text-white" />
                                    </div>
                                    <div className="text-left">
                                      <h4 className="font-medium text-lg text-foreground dark:text-foreground">{topic.title}</h4>
                                      {topic.content && (
                                        <p className="text-base text-muted-foreground dark:text-foreground/70 mt-0.5 line-clamp-1">
                                          {topic.content}
                                        </p>
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <Badge variant="outline" className="text-xs px-1.5 py-0.5 rounded-md dark:bg-background/50 dark:border-border dark:text-foreground/80">
                                      {topic.lessons.length}
                                    </Badge>
                                    {openTopics.includes(topic.id) ? (
                                      <ChevronDown className="w-3.5 h-3.5 text-muted-foreground dark:text-foreground/70" />
                                    ) : (
                                      <ChevronRight className="w-3.5 h-3.5 text-muted-foreground dark:text-foreground/70" />
                                    )}
                                  </div>
                                </div>
                              </CardHeader>
                              ...
                              <CardContent className="pt-0 pb-2 px-3">
                                <div className="space-y-1.5">
                                  {topic.lessons.map((lesson) => (
                                     <div
                                      key={lesson.id}
                                      className="flex items-center justify-between p-2.5 bg-white/60 dark:bg-card/40 rounded-lg border-0 hover:bg-white/80 dark:hover:bg-card/60 transition-colors cursor-pointer group"
                                      onClick={() => setSelectedLesson(lesson)}
                                    >
                            <div className="flex items-center gap-2.5">
                                        <div className="w-5 h-5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-md flex items-center justify-center shadow-md">
                                      ...
                                      <div className="flex items-center gap-1 flex-shrink-0">
                                        {lesson.media && lesson.media.length > 0 && (
                                          <Badge variant="outline" className="text-xs px-1 py-0.5 rounded dark:bg-background/50 dark:border-border dark:text-foreground/80">
                                            {lesson.media.length}
                                          </Badge>
                                        )}
      ...
      <Dialog open={!!selectedLesson} onOpenChange={() => setSelectedLesson(null)}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto dark:bg-card transition-colors duration-300">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center gap-3 text-foreground dark:text-foreground">
            ...
            <div className="space-y-6">
              ...
              {selectedLesson.content && (
                <div className="prose max-w-none">
                  <div className="bg-muted/20 dark:bg-muted/10 p-6 rounded-lg border dark:border-border transition-colors duration-300">
                    <h4 className="text-lg font-semibold mb-3 text-foreground dark:text-foreground">محتوى الدرس:</h4>
                    <div className="text-muted-foreground dark:text-foreground/80 leading-relaxed whitespace-pre-wrap">
                      {selectedLesson.content}
                    </div>
                  </div>
                </div>
              ...
              {selectedLesson.media && selectedLesson.media.length > 0 && (
                <div>
                  <h4 className="text-lg font-semibold mb-4 text-foreground dark:text-foreground">الوسائط المرفقة:</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedLesson.media.map((media) => (
                      <Card 
                        key={media.id} 
                        className="cursor-pointer hover:shadow-md dark:hover:shadow-primary/10 transition-shadow border-2 hover:border-primary/50 dark:border-border dark:hover:border-primary/50 dark:bg-card/60"
                        onClick={() => setPreviewMedia(media)}
                      >
                        <CardContent className="p-4">
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-lg flex items-center justify-center shadow-lg ${getMediaColor(media.media_type)}`}>
                              {getMediaIcon(media.media_type)}
                            </div>
                            <div className="flex-1">
                              <h5 className="font-medium text-base mb-1 text-foreground dark:text-foreground">{media.file_name}</h5>
                              <div className="flex items-center gap-2">
                                <Badge variant="secondary" className="text-xs dark:bg-secondary/20 dark:text-foreground/80">
                                ...
                                <Button size="sm" variant="ghost" className="h-6 px-2 dark:hover:bg-accent/50">
