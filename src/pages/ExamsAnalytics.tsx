...
interface ExamsAnalyticsProps {
  gradeLevel?: string;
}

const ExamsAnalytics: React.FC<ExamsAnalyticsProps> = ({ gradeLevel }) => {
  const { user } = useAuth();
  ...
  const filteredExams = useMemo(() => {
    ...
    if (!gradeLevel) return data.exams;
    
    return data.exams.filter(exam => 
      exam.grade_levels.includes(gradeLevel) || 
      exam.grade_levels.includes(`الصف ${gradeLevel === '10' ? 'العاشر' : gradeLevel === '11' ? 'الحادي عشر' : 'الثاني عشر'}`)
    );
  }, [data?.exams, gradeLevel]);

  // حساب إحصائيات الصف المحدد
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatsCard
          ...
          value={gradeStats.activeExams.toString()}
          icon={FileText}
        ...
        <StatsCard
          ...
          value={gradeStats.totalAttempts.toString()}
          icon={Users}
        ...
        <StatsCard
          title="متوسط النتائج"
          value={gradeStats.avgScore !== null ? `${gradeStats.avgScore.toFixed(1)}%` : 'لا توجد بيانات'}
          icon={TrendingUp}
      ...
      {/* جدول الامتحانات التفصيلي */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileText className="h-5 w-5" />
            تفاصيل الامتحانات {gradeLevel && `- الصف ${gradeLevel === '10' ? 'العاشر' : gradeLevel === '11' ? 'الحادي عشر' : 'الثاني عشر'}`}
          </CardTitle>
        </CardHeader>
        <CardContent>
