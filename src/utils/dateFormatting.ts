...
/**
 * Formats a number with English numerals and thousands separators
 ...
 * @returns Formatted duration (e.g., "2h 30m", "45m", "< 1m")
 */
 ...
 * @returns Formatted date (e.g., "14:30", "Mon 14")
 */
export const formatDateChart = (date: Date | string, format: 'time' | 'date' = 'time'): string => {
  const dateObj = typeof date === 'string' ? new Date(date) : date;
  ...
  return formatDateNumeric(dateObj);
};
