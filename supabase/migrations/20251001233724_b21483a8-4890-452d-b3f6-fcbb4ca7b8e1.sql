-- إصلاح دالة get_student_total_points بإضافة ::uuid للتحويل الصريح
...
BEGIN
  ...
  IF student_grade = '10' THEN
    ...
    WHERE gs.title LIKE '%تركيبة الحاسوب%'
      AND EXISTS (
        ...
        WHERE sp.student_id = student_uuid::uuid
    ...
    WHERE pgp.player_id = student_uuid::uuid
  ...
  RETURN COALESCE(
    (SELECT SUM(points_earned) FROM public.student_progress WHERE student_id = student_uuid::uuid) +
    (SELECT SUM(points_value) FROM public.student_achievements WHERE student_id = student_uuid::uuid) +
    (SELECT SUM(points_earned) FROM public.student_activity_log WHERE student_id = student_uuid::uuid),
    0
  );
END;
$function$;

-- إصلاح دالة get_student_dashboard_stats بإضافة ::uuid للتحويل الصريح
...
-- إصلاح دالة get_student_assigned_grade بإضافة ::uuid للتحويل الصريح
