-- إصلاح دالة get_available_exams - استخدام الأعمدة الصحيحة
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      ...
      WHEN e.start_datetime IS NOT NULL AND e.start_datetime > now() THEN false
      WHEN e.end_datetime IS NOT NULL AND e.end_datetime < now() THEN false
    ...
    CASE
      ...
      WHEN e.start_datetime IS NOT NULL AND e.start_datetime > now() THEN 'upcoming'
      WHEN e.end_datetime IS NOT NULL AND e.end_datetime < now() THEN 'expired'
  ...
  WHERE e.status = 'active'
    AND (v_student_grade = ANY(e.grade_levels) OR e.grade_levels IS NULL OR array_length(e.grade_levels, 1) IS NULL)
  GROUP BY e.id
  ORDER BY 
    CASE 
      WHEN e.start_datetime IS NOT NULL AND e.start_datetime > now() THEN e.start_datetime
      ELSE e.created_at
    END DESC;
END;
$function$;