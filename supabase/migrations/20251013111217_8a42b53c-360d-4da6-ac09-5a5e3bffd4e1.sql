-- حذف الدالة القديمة
DROP FUNCTION IF EXISTS public.generate_exam_questions(uuid, uuid);

-- إنشاء دالة محسّنة تدعم جميع مصادر الأسئلة
...
COMMENT ON FUNCTION public.generate_exam_questions IS 'توليد أسئلة الامتحان حسب المصدر المحدد: random (عشوائي), specific_sections (أقسام محددة), my_questions (أسئلة المعلم)';