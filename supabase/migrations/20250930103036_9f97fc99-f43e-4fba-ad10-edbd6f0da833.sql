-- دالة محسّنة لجلب تفاصيل تقدم الطلاب
    ...
    SELECT 
      ...
      jsonb_agg(
        jsonb_build_object(
          'project_id', gmp.id,
          'title', gmp.title,
          'progress_percentage', gmp.progress_percentage,
          'status', gmp.status,
          'last_saved_at', gmp.last_saved_at
        )
      ) as projects
    ...
    SELECT 
      ...
      jsonb_agg(
        jsonb_build_object(
          'project_id', gfp.id,
          'title', gfp.title,
          'progress_percentage', gfp.progress_percentage,
          'status', gfp.status,
          'last_saved_at', gfp.last_saved_at
        )
      ) as projects
