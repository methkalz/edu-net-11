-- إصلاح دالة submit_exam_attempt لحل مشكلة المقارنة الخاطئة
...
-- دالة لإعادة حساب نتائج محاولة موجودة
CREATE OR REPLACE FUNCTION public.recalculate_exam_attempt(p_attempt_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  -- إعادة تعيين الحالة إلى in_progress مؤقتاً
  UPDATE public.exam_attempts
  SET status = 'in_progress'
  WHERE id = p_attempt_id;
  
  -- استدعاء دالة submit_exam_attempt لإعادة الحساب
  RETURN public.submit_exam_attempt(p_attempt_id);
END;
$function$;