...
-- 4. إصلاح RLS policies لجدول pair_matching_results
DROP POLICY IF EXISTS "Players can manage their own results" ON public.pair_matching_results;
DROP POLICY IF EXISTS "Players can view their own results" ON public.pair_matching_results;

CREATE POLICY "Players can manage their own results" 
...
-- 5. تهيئة التقدم لجميع الطلاب الموجودين بدون تقدم مسجل
...
-- 6. التأكد من فتح اللعبة الأولى لجميع الطلاب
