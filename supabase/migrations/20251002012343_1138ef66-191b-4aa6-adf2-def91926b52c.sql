-- تحديث دالة get_student_total_points لحساب نقاط الصف الثاني عشر بشكل صحيح
...
BEGIN
  ...
  -- Grade 12 logic - حساب النقاط من profiles + نقاط المحتوى المكتمل
  IF student_grade = '12' THEN
    -- حساب النقاط من student_progress
    SELECT COALESCE(SUM(points_earned), 0) INTO progress_points
    FROM student_progress
    WHERE student_id = student_uuid
      AND progress_percentage = 100;
    
    -- النقاط الإجمالية = النقاط الأساسية + نقاط التقدم
    total_points := base_points + progress_points;
