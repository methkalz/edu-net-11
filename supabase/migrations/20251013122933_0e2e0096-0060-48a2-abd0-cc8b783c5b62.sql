-- إصلاح دالة submit_exam_attempt لاحتساب النتائج من الأسئلة الفعلية في الامتحان
...
BEGIN
  ...
  -- حساب النتيجة لكل سؤال من جدول exam_questions (الأسئلة الفعلية للامتحان)
  FOR v_question IN
    SELECT 
  ...
  LOOP
    ...
    -- الحصول على إجابة الطالب (المعرف)
    ...
    -- تحويل معرف الإجابة إلى النص الكامل للمقارنة
    v_student_answer_text := NULL;
    
    IF v_question.choices IS NOT NULL THEN
      -- البحث عن النص المقابل لمعرف الإجابة في الخيارات
      ...
      END LOOP;
    ELSE
      -- للأسئلة المفتوحة، استخدم الإجابة مباشرة
      v_student_answer_text := v_student_answer;
    END IF;
    
    -- مقارنة الإجابات (النص مع النص)
    IF v_student_answer_text IS NOT NULL AND 
       v_correct_answer IS NOT NULL AND
       LOWER(TRIM(v_student_answer_text)) = LOWER(TRIM(v_correct_answer)) THEN
