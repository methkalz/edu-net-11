-- إصلاح احتساب الإجابات الصحيحة من بنك الأسئلة
-- تحويل choice_id إلى النص المقابل عند المقارنة
...
BEGIN
  ...
  IF v_questions_from_table = 0 THEN
    ...
    LOOP
      ...
      -- الحصول على الإجابة الصحيحة من questions_data
      ...
      -- معالجة أسئلة multiple_choice من question_bank
      v_is_correct := FALSE;
      
      IF v_question.question_type = 'multiple_choice' AND v_question.source = 'bank' THEN
        -- تحويل choice_id إلى النص المقابل
        ...
        END IF;
      ELSE
        -- المقارنة المباشرة لباقي الأنواع (true_false, أسئلتي)
  ...
  ELSE
    ...
    LOOP
      ...
      IF v_question.question_type = 'multiple_choice' AND v_question.question_source = 'bank' THEN
        -- تحويل choice_id إلى النص المقابل
        v_student_answer_text := NULL;
        FOR v_choice IN SELECT * FROM jsonb_array_elements(v_question.choices)
        LOOP
