-- توحيد نقاط جميع الأسئلة إلى 1 نقطة
-- الخطوة 1: تحديث بنك الأسئلة
UPDATE question_bank 
SET points = 1 
WHERE points != 1;

-- الخطوة 2: تحديث أسئلة المعلمين المخصصة
UPDATE teacher_custom_questions 
SET points = 1 
WHERE points != 1;

-- الخطوة 3: تعديل دالة submit_exam_attempt لحساب النسبة بناءً على عدد الإجابات الصحيحة
...
BEGIN
  ...
  -- حساب النسبة المئوية بناءً على عدد الإجابات الصحيحة (وليس النقاط)
